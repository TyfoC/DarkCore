...
size_t StringUtils::FindLastSubstr(const char* source, const char* substr, size_t offset) {
	if (!offset) offset = GetLength(source) - 1;
	size_t srcLength = GetLength(source);
	size_t subLength = GetLength(substr);

	bool equal;
	srcLength -= subLength;
	for (ptrdiff_t index = (ptrdiff_t)offset; index >= 0; index--) {
		equal = true;
		for (size_t i = 0; substr[i]; i++) if (source[index + i] != substr[i]) {
			equal = false;
			break;
		}
